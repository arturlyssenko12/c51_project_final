...
sweep_configuration = {
    "name": "e2gnn_hessian-distill-sweep",
    "method": "bayes",
    ...
    "parameters": {
        ...
        "HESSIAN_WEIGHT": {"values": [100.0, 200.0, 400.0, 800.0]},
...
def evaluate(loader, model, device):
    ...
    for batch in tqdm(loader, desc="Evaluating"):
        ...
        data.pos = pos
        _, f_new = model(Batch.from_data_list([data]))

        n_atoms = pos.size(0)
        n_coords = n_atoms * 3
        row_idx = random.randint(0, n_coords - 1)

        pred_row = compute_hessian_rows(f_new, pos, [row_idx])[0]
        true_row = batch.hessian[row_idx, :n_coords]

        h_true.append(true_row.cpu())
        h_pred.append(pred_row.cpu())
    ...
    h_loss = ((torch.stack(h_true).view(-1) - torch.stack(h_pred).view(-1))**2).mean().item()
    return e_loss, f_loss, h_loss

def main():
    wandb.init(project="e2gnn_hessian-distill-sweep")
    config = wandb.config
    ...
    target_len = min(len(target_dict["energy"]), len(target_dict["forces"]), len(target_dict["hessian"]))
    ...
    val_loader = DataLoader(AtomsDataset(atoms_val, target_val, device), batch_size=1, shuffle=False, collate_fn=collate_fn_e2gnn)

    for epoch in range(config.EPOCHS):
        student.train()
        total_loss = total_energy_loss = total_force_loss = total_hessian_loss = 0
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{config.EPOCHS}"):
            ...
            pred_hessian_rows, true_hessian_rows = [], []

            for i in range(len(batch.ptr) - 1):
                start, end = batch.ptr[i].item(), batch.ptr[i + 1].item()
                n_coords = (end - start) * 3
                pos_i = batch.pos[start:end].detach().clone().requires_grad_(True)
                data_i = batch.to_data_list()[i]
                data_i.pos = pos_i
                _, forces_i = student(Batch.from_data_list([data_i]))
                row_idx = random.randint(0, n_coords - 1)
                pred_row = compute_hessian_rows(forces_i, pos_i, [row_idx])[0]
                true_row = batch.hessian[i * n_coords + row_idx, :n_coords]
                pred_hessian_rows.append(pred_row)
                true_hessian_rows.append(true_row)

            hessian_loss = sum(loss_fn_hessian(p, t.detach()) for p, t in zip(pred_hessian_rows, true_hessian_rows)) / len(pred_hessian_rows)
            loss = config.ENERGY_WEIGHT * energy_loss + config.FORCE_WEIGHT * force_loss + config.HESSIAN_WEIGHT * hessian_loss
            optimizer.zero_grad()
            loss.backward()
        ...
        val_e, val_f, val_h = evaluate(val_loader, ema_student.module, device)
        wandb.log({
            "val_loss_energy": val_e,
            "val_loss_force": val_f,
            "val_loss_hessian": val_h,
            "val_loss_total": val_e * config.ENERGY_WEIGHT + val_f * config.FORCE_WEIGHT + val_h * config.HESSIAN_WEIGHT,
            "train_loss_energy": total_energy_loss / len(train_loader),
            "train_loss_force": total_force_loss / len(train_loader),
            "train_loss_hessian": total_hessian_loss / len(train_loader),
            "train_loss_total": total_loss / len(train_loader)
        })

if __name__ == "__main__":
    sweep_id = wandb.sweep(sweep=sweep_configuration, project="e2gnn_hessian-distill-sweep")
